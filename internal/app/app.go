...
func (app *App) repairSlaveOfflineMode(host string, state *nodestate.NodeState, masterNode *mysql.Node, masterState *nodestate.NodeState) {
	...
	// resetup offline host, if lag is too large
	if state.IsOffline && !masterState.IsReadOnly && *state.SlaveState.ReplicationLag > app.config.ResetupHostLag.Seconds() {
		err := app.SetRecovery(host)
		if err != nil {
			app.logger.Errorf("repair: failed to set recovery on host %s: %s", host, err)
		}

		app.logger.Infof("repair: slave %s set to recovery, because ReplicationLag (%f s) > ResetupHostLag (%v)",
			host, *state.SlaveState.ReplicationLag, app.config.ResetupHostLag.Seconds())
