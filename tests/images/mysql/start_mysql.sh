...
if [ ! -f /etc/mysql/slave.sql ]; then
    ...
    cat <<EOF > /etc/mysql/slave.sql
        ...
        CHANGE MASTER TO MASTER_HOST = '$MYSQL_MASTER', MASTER_USER = 'repl', MASTER_PASSWORD = 'repl_pwd', MASTER_AUTO_POSITION = 1, MASTER_CONNECT_RETRY = 1, MASTER_RETRY_COUNT = 100500, GET_MASTER_PUBLIC_KEY=1 FOR CHANNEL '';
        START SLAVE;
EOF
    else
