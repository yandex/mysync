Feature: source/replica semisync dialect
  ...
  Scenario: mysync manages semisync with source/replica plugins
    ...
    When I run SQL on mysql host "mysql1"
      """
      SELECT LEFT(VERSION(), 3) AS Version
      """
    ...
    When I run SQL on mysql host "mysql1"
      """
      SELECT User, plugin AS Plugin
      FROM mysql.user
      WHERE User IN ('admin', 'repl', 'user') AND Host = '%'
      ORDER BY User
      """
    Then SQL result should match json_exactly
      """
      [
        {"User":"admin","Plugin":"caching_sha2_password"},
        {"User":"repl","Plugin":"caching_sha2_password"},
        {"User":"user","Plugin":"caching_sha2_password"}
      ]
      """
